...
export default function JourneyRouteMap({ stops }: JourneyRouteMapProps) {
  ...
  const initMap = useCallback(async () => {
    ...
    // ── Fully destroy any existing map instance ─────────────
    ...
    // Reset container HTML so Leaflet sees a fresh element
    containerRef.current.innerHTML = ''

    const L = (await import('leaflet')).default

    // Fix Leaflet's broken default icon paths in Next.js
    // (avoids the missing marker icon 404s)
    ...
    const maps = L.map(containerRef.current, {
      ...
      // Disable scroll zoom to avoid conflicts with page scroll
    ...
    let drawn = 1
    const SPEED = 1 // 1 pt/frame → slowest, most cinematic

    const animate = () => {
      if (!mapInstanceRef.current) return // map was destroyed
      ...
      if (drawn < allPoints.length) {
        ...
        // Animation finished — hide dot, show replay
        dotMarker.remove()
        setAnimDone(true)
      }
    }
  ...
  return (
    <div className="rmap-wrapper">
      {/* Map canvas — key is NOT on this div, we manage the instance manually */}
        ...
        <div className="rmap-popup-wrap">
          <div className="rmap-popup">
            ...
            <div className="rmap-popup-body">
              ...
              {activeStop.note && (
                <p className="rmap-popup-note">{activeStop.note}</p>
              )}
            </div>
            <button
              className="rmap-popup-close"
      ...
      {/* Replay — only after animation finishes */}
      {animDone && (
        <button
          className="rmap-replay"
          onClick={() => {
            setReplayKey((k) => k + 1)
          }}
        >
      ...
      <style jsx global>{`
        ...
        .rmap-marker:hover .rmap-marker-ring {
          background: rgba(232, 134, 90, 0.45);
          animation: none;
        }
        .rmap-marker-num {
          position: relative;
        ...
        .rmap-marker:hover .rmap-marker-num {
          transform: scale(1.2);
        }
        ...
        .rmap-stop-item:hover {
          background: rgba(255, 255, 255, 0.05);
        }
        .rmap-stop-item--active {
          background: rgba(232, 134, 90, 0.13);
        }
        .rmap-stop-num {
          width: 18px;
          height: 18px;
        ...
        .rmap-popup-close:hover {
          background: rgba(232, 134, 90, 0.55);
          color: #fff;
        }
        ...
        .rmap-replay {
          ...
          border: 1px solid rgba(232, 134, 90, 0.35);
          color: #e8865a;
          font-size: 0.7rem;
          font-weight: 600;
