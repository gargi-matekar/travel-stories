...
export default async function TripPage({ params }: Props) {
  ...
  // Replace the allStops line with city-level data:
  const tripCities = trip.stories.map((s) => ({
    id: s.id,
    city: s.city,
    state: s.state,
    country: s.country,
    ...
    <main style={{ background: "transparent", color: "var(--text-primary)" }}>
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-6 pb-16">
          <p className="text-xs tracking-[0.4em] uppercase mb-4 text-white/50">
            Journey
          </p>
          <h1
      ...
      {tripCities.length > 0 && (
        ...
        >
          <div className="max-w-5xl mx-auto px-6">
            ...
            <TripRouteMap cities={tripCities} />
          </div>
        </section>
      )}
      ...
      {trip.expenses.length > 0 && (
        <ExpenseBreakdown expenses={trip.expenses} totalCost={totalExpense} />
      )}
    </main>
  );
}
